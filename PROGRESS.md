...
## 최근 완료 (2026-07-23 저녁) — 홈 로딩 개편 A·B·C: 콜드 58초 → 캐시 셸 2ms
- **배경(사용자 보고)**: 홈 화면 아이콘(PWA)으로 열면 홈 탭까지 너무 오래 걸림.
  실측 = 프로덕션 콜드 58.5초/웜 3.8초. 원인 4겹(전량 로드·매요청 재판정·SW 부재·인라인 CSS).
- **A** 목록 컬럼 다이어트(`appraisal_notes` 14.1MB 제외, `_RIGHTS_LIST_COLS`) +
  파생 맵 지문 캐시(`_derived_maps` — summarize 1.6초를 데이터 변경 시에만).
- **B** 서비스워커 신설(`static/sw.js`): 탭 8종 SWR(캐시 셸 즉시 + 내용 달라졌을 때만 갱신 필),
  상세·쿼리목록·API 무개입(최신성 우선). Playwright 실측: 캐시 서빙 **2ms**, 오프라인 홈 표시.
- **C** 인라인 CSS 61KB → `base.css?v=<해시>` 외부화(immutable). base.html 65KB→3.4KB.
  부산물: 'gapmeter' 허상 테스트 3건 발견·정리 + 죽은 `meter=` 배선 3곳 제거.
- 테스트 786 passed·ruff 클린. 상세 versions.md 18:03 항목. **미커밋**(다른 세션과 조율).
