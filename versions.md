...
- **detail.html**: ① 판정 뱃지 줄(.dvband — 인수권리/대항력(priority 판정 연동)/신고임차인0/
  ...
  레이아웃은 불변.
- **증거**: pytest 915 passed / 실패 2건(test_index_filter_form_and_selection·test_index_area_filter,
  오피스텔 샘플 "강남역삼푸르지오시티" 홈 노출)은 **stash 검증으로 본 변경 무관·기존 실패 확인**
  (다른 세션 변경분 영향 추정 — 원인 미추적). 실서버(:8124 임시) 렌더 캡처
  `경매-비포애프터\20260725_상세개편_실이식_비포애프터.png`.
