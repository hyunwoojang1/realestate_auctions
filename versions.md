...
원래 SWR(캐시 먼저)은 콜드 58초 시절의 방어였는데 WarmPing+워밍 도입 후 웜 응답이
0.3~0.6초로 바뀌어 전제가 무너져 있었다(실측: / 0.29s · /map 0.30s · **/sold 3.5s**).

**무엇**:
- **/sold 3.5초의 진범**: store_rest.fetch_sold 만 TTL 캐시가 없어 매 요청 17k행 전량
  재로드(scored·rights·naver 는 전부 캐시 있음 — sold 만 나중에 추가되며 누락).
  _sold_cache 추가(+conftest 테스트 격리 — 캐시가 env 확인보다 먼저 히트하므로).
- **sw.js v6 — networkFirst 전환**: 서버를 3초까지 기다려 오면 최신 서빙(=배포 즉시
  반영), 늦으면(콜드·오프라인) 캐시 폴백 + 기존 정직성 계약('저장된 화면' 표식·
  '새 데이터 도착' 필·실패 통지) 그대로. 최신을 직접 서빙한 경우엔 필 미발화(오탐 방지).
  앞으로 화면 개편 배포에 VERSION 범프 불필요(SW 로직 변경 때만).

**증거(배포 후 프로덕션 실측)**: sw.js v6 서빙 확인 · /sold 3.5s→**0.78~0.86s**(캐시
히트, 4배↑) · 화면 정상(새 공식 1·옛 문단 0). 첫 요청(캐시 채움 7.2s)은 warm-ping 에
/sold 추가로 선점 워밍(사용자가 만날 일 없게) — 실행 검증 완료.

## 2026-08-25 19:25 KST — 🔁 SW v5 — 폰 캐시가 옛 낙찰결과 화면을 계속 서빙 (사용자 지적)

**전말**: 목록 개편 배포는 정상(프로덕션 실측 — 새 공식 1·옛 문단 0)이었으나 사용자
폰에는 옛 화면이 계속 표시. 원인 = 서비스워커 stale-while-revalidate 가 이전 방문 캐시본을
우선 서빙(설계 동작이지만 개편 직후엔 체감 '반영 안 됨'). VERSION v4→v5 로 activate 시
구 캐시 전체 삭제를 강제. 테스트의 버전 단언을 상수 일치 → '>=4' 로 완화(매 범프마다
테스트 수정하는 구조 제거).

**교훈(프로세스)**: "배포 완료" 보고 전 ①프로덕션 응답 실측 ②**클라이언트 캐시 경로**
(SW·private cache)까지 검증. 서버가 새 걸 줘도 폰이 옛 걸 보여주면 사용자에겐 미배포다.

## 2026-08-25 19:00 KST — ✂️ 낙찰 결과 목록 설명 철거 — 공식 한 줄 + 접힘

**배경**: 사용자 — 상세만 고쳤더니 낙찰 결과 **목록**의 8줄 설명 문단이 그대로였다.

**무엇**(sold.html):
- page-lead 8줄 → **"시세 대비 = 지금 시세 하한 − 낙찰가" 한 줄** + '기준 자세히' 접힘
  (4개 요점으로 압축: 같은 단지·같은 평형만 / 감정가율 30% 미만 비교 불가 /
  법원 공개 금액만 / 시점 차 존재).
- covnote 부연·하단 foot 문단 삭제, 검색 카드 부제 축약, 낙찰가 셀 '미공개'→'미수집'
  (칩과 용어 통일 — 매각결과 API 도입 후 정정된 표현).

**증거**: test_sold_listings+pwa_shell 72 passed, 실DB 렌더에서 공식·접힘 확인
·철거 문구 잔존 0.

## 2026-08-25 18:40 KST — 🧾 계산서 '공식 먼저' 개편 + 웹푸시 키 버그 수정·실발송 성공

**① 웹푸시 마무리**: 사용자 폰 구독 등록 확인(Apple 엔드포인트 1건) → 발송 실패
(pywebpush 는 PEM 이 아니라 **raw base64url 개인키**를 원함 — ASN.1 파싱 실측) →
vapid.json 에 private_raw 추가·발송부 수정 → **테스트 푸시 1대 발송 성공**.

**② 계산서 정보 과다(사용자: "설명이 많아 공식이 안 보인다")**:
- 상세 '가격·수익 계산서' 맨 위에 **공식 박스** 신설 — 실숫자 대입 한 줄:
  "차익 1.17억 = 시세 하한 3.7억 − 취득원가 2.47억 − 인수 588만".
  등식은 전 행 검증(2,849/2,849 일치) 후 도입 — 안 맞는 등식 표시 금지.
- '산출 근거' 세부(매칭수·신뢰계수·비교군·저층 보정·세금 주석)는 **기본 접힘**
  (details/summary). 정직성 배너(표본 낮은 신뢰·밴드 과대)는 안전 정보라 접지 않음.
- base.css 에 .dformula/.dfold 스타일(해시 캐시버스팅 자동).

**증거**: 실DB 렌더 2케이스(인수 유/무) 공식·접힘 확인. 게이트 전체 pytest.
...
`$RightsLimit건` 이라는 '한글 포함 변수명'으로 파싱돼 빈 문자열로 찍혔다(`${}` 로 수정).
실인자 전달(`--limit $RightsLimit`)은 공백 분리라 정상이었음 — 표시만의 버그지만,
오늘 로그의 "상위 건" 이 실제 인자 누락으로 오인될 뻔했다(원인 규명에 시간 소모).

**참고 — 크롤 진행 실측(14:05)**: [1/5] 완료, **[1.5/5] 실낙찰가 백필 첫 자동 실행 성공
(채움 269 · Supabase 병합 1,319건)**, [2/5] 권리 크롤 진행 중.

## 2026-08-25 14:00 KST — 🔓 프로덕션 7일 stale 근본 원인 — 지분 검출 미탐 1건이 미러를 볼모로

**배경**: 사용자 보고 "데이터 기준 7.3일 전 수집분". 조사 결과 — 8/17부터 **품질 게이트
FAIL 이 클라우드 미러를 매일 차단**해 왔다(크롤 자체는 exit 0 완주, 로컬만 갱신).
위반은 단 1건: 2025타경56099(푸르뫼금강에스쁘아) — maejibun "지분 2분의 1 전부"의
**어순 반대** 표기를 검출 정규식(분수→지분 순서만)이 미탐 → 1/2 지분에 온전가 시세
1.95억이 붙음 → 게이트(비고 백업 키워드)가 잡아 미러 차단. 게이트는 알고 검출은 모르는
불일치가 7일간 프로덕션을 볼모로 잡은 구조.

**무엇**(`is_partial_share` 강화 — 전수 63,428행 실측 경계):
- 어순반대 정규식 `지분…N분의M` 추가(매치 4,463건 전부 진성, '전원' 충돌 0)
- 비고 명시 키워드 '지분매각' 추가(4,958건 전부 진성) — 게이트 백업과 검출 정렬
- '전원'=온전 우선 규칙 유지(모순 1건 실측 — maejibun 명시 신뢰, 주석 문서화)
- 회귀 테스트 4종 추가(실사고 표기 그대로) — 총 16/16 passed

**영향(전수)**: 현재 시세 보유 중 3건이 재채점 시 지분으로 강등·시세 회수(허구 차익 제거).
진행 중인 오늘 크롤의 [5/5] 재채점이 새 프로세스로 수정본을 로드 → 게이트 PASS →
미러 재개 → stale 배너 해소 예상(~16시). 확인 후 재보고. README §6 지분 경계표 동기화 + DOC_SYNC_QUEUE 소화 완료(같은 턴).

**추천 푸시 미수신 건**: ntfy 발행은 200 OK — 수신 측(폰 ntfy 앱 미설치/미구독) 문제로
추정, 사용자 안내 진행.

## 2026-08-25 13:20 KST — 📲 추천 물건 푸시 알림 신설 (deploy/notify_picks.py)

**배경**: 사용자 요구 — 홈 화면 앱은 깔았는데 들어갈 계기가 없다. "어느 지역 아파트가
감정가 얼마인데 최저 얼마로 나왔다" 알림이 오면 열어보게 된다.

**무엇**:
- `deploy/notify_picks.py` — 선별은 **홈 추천과 동일 게이트**(digest.top_listings:
  보수차익>0·같은단지 같은평형 표본≥5·위험 제외·인수부담 제외) + 당일 마감 제외 +
  **기발송 30일 제외**(data/notified_picks.json — 매일 같은 1등 반복이면 알림을 끄게 된다).
  푸시 1건에 상위 3건 요약, **클릭 시 1위 물건 프로덕션 상세 직행**(ntfy click URL).
  채널은 기존 운영 ntfy 토픽 재사용, 실패 비차단 + ALERTS.log 병기.
- refresh-daily [5/5] 재채점 **직후** 배선 — 그날 보강 반영된 최종 등급으로 발송.
- tests/test_notify_picks.py 6케이스(포맷·지역 라벨·마감/기발송 제외·클릭 URL 인코딩).

**증거**: 실DB 발송 실측 — "용인 버들치마을성복자이1차 25.5평 — 감정 8.9억 → 최저
3.1억(유찰 3회) · 보수차익 +5.3억 · 기일 08-31(D-6)" 외 2건, ntfy 200 + 이력 기록.
테스트 6/6 passed·ruff 클린.

**참고**: 낮 12:40 재발화한 크롤은 이 작업과 병행 진행 중([1/5] 정상 순항).
...
문서화·보고해 왔다. 사용자 반문("정말 못 구해오는 게 맞아?")으로 재정찰 —
**틀렸었다.** 활성 목록 API(searchControllerMain)만 본 결론이었고, 대법원 사이트의
별도 화면 **매각결과검색**(PGJ158M01/M02)의 전용 API `selectDspslSchdRsltSrch.on` 이
최근 매각기일의 정상 낙찰가(maeAmt)를 공개한다. WebSquare 화면 XML 정적 분석(밴 무관)으로
요청 스키마를 복원하고 라이브 1콜로 검증(statNum='3' 고정, auctnGdsStatCd='04'=매각만
→ 전 행 maeAmt>0).

**무엇**:
- `courtauction_client.sold_results()` — 법원 1곳 매각결과 1페이지(요청 골격 상수화).
  기존 지터·403 즉시중단·일일예산·킬스위치 전부 상속.
- `deploy/crawl_sold_results.py` — 법원 57곳 순회(법원당 1콜+α) → saNo→'YYYY타경N' 변환
  (로컬 raw 500/500 대조 검증) → **빈 sold_price 만** 채움(기존 값 불변 = 정직성 계약,
  sold_evidence='dspslRslt' 로 재매각 이력 경로와 출처 구분) → --mirror 시 Supabase 병합
  (전체 컬럼 행으로 부분행 삽입 방지, 멱등).
- refresh-daily [1.5/5] 배선(비차단 — 실패해도 다음날 창 안 재수집), 템플릿 출처 라벨
  ('법원 매각결과' 칩) + **"법원은 정상 낙찰가를 공개하지 않습니다" 문구 정정**(상세·README).
- tests/test_sold_results.py 4케이스(변환 규칙·중복 제거·빈 값만 채움·기존값 불변).

**증거(전수)**: 첫 실전 실행 — 법원 57곳, 후보 1,245건 → **채움 924 · 기존값 보존 146 ·
로컬 미보존 175**. 누적 실낙찰가 409→**1,465건**(3.6배). Supabase 병합 1,056건 확인.
공개 창은 직전 기일 ~2주(실측) — 과거 소급 불가, 매일 수집으로 창 유지가 핵심.

**다음**: 내일 05:30 첫 자동 실행분 확인(스케줄러 복구 검증과 동일 시점).
