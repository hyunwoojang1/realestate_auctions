...
# (QA 2026-07-26) 이름 뒤 잔여 한글(group3)도 캡처한다 — 종전엔 {2,4}가 '한국전력공사'의 앞
# 4자(한국전력)만 떼어 법인 접미사 가드가 전체 단어를 못 보고 '[성명]공사'로 훼손했다(실측
# 22행 저장 오염). group3 은 법인 전체단어 판정에만 쓰고, 마스킹 시엔 원문 그대로 되돌린다.
_PII_CONTEXT_RE = re.compile(
    r"(주택임차권자|유치권신고인|유치권자|임차권자|근저당권설정자|근저당권자|가압류권자"
    r"|전세권자|지상권자|전세권설정자|채권자|채무자|소유자|공유자|임차인|임대인"
    r"|점유자|신청인|배우자|상속인|연고자)[\s:：]+([가-힣]{2,4})([가-힣]*)"
    r"((?:\s*[,·]\s*[가-힣]{2,4}[가-힣]*)*)"
)
# 나열 꼬리 안의 개별 항목 — (구분자, 이름, 잔여한글) 로 쪼개 이름만 치환.
_PII_NAME_LIST_RE = re.compile(r"(\s*[,·]\s*)([가-힣]{2,4})([가-힣]*)")
...
_NOT_A_NAME = frozenset({
    ...
    # (QA 2026-07-26) 역할어 뒤 서류·행위어 — "채권자 제출 보정서"·"채권자 확약서 제출"의
    # 제출/확약서가 이름으로 오인돼 '[성명] 보정서'로 훼손됐다(실측 저장 오염).
    "제출", "제출한", "확약서", "보정서", "동의서", "포기서", "신고서", "서면",
    # (QA 2026-07-26) "채권자 대리인 법무법인 …"의 대리인, "주식회사" 오염행의 주식 — 역할어·법인어.
    "대리인", "주식",
...
_CORP_SUFFIX = (
    ...
    # (QA 2026-07-26) 회사=주식회사·유한회사 등 총칭, 법인=영농조합법인·재단법인 등 총칭 —
    # '서울보증보험주식회사'·'○○영농조합법인' 전체단어 판정용(아래 _corp_word).
    "회사", "법인", "중앙회",
    # (2026-07-31) '지상권자 한국전력' 이 커밋 게이트에 실명 의심으로 걸렸다 — 한전은 법인이고,
    # 송전선로 지상권은 매수인이 알아야 할 **공시정보**라 [성명]으로 지우면 신호가 사라진다.
    # '공사'가 붙은 '한국전력공사'는 이미 보존됐는데 약칭 '한국전력'만 구멍이었다.
    # 한국어 성명이 '전력'으로 끝나는 경우는 없어 실명을 삼킬 위험이 없다.
    "전력",
)
# 성명/명사 뒤에 붙는 조사 — 제외어 판정 시 떼어내고 본다("전원의"→'전원'=제외, "전원철"→실명).
_JOSA = ("으로부터", "으로", "에게", "에서", "로부터", "의", "은", "는", "이", "가", "도", "만", "에",
         "와", "과", "로", "을", "를", "께", "님", "부터")
...
def _corp_word(word: str) -> bool:
    """단어 전체(조사 제거 후)가 법인·기관명으로 끝나는가 — '한국전력공사', '영농조합법인으로부터'.

    (QA 2026-07-26) 이름 정규식이 앞 2~4자만 잘라 판정하면 5자+ 법인명의 접미사를 놓친다 —
    전체 단어로 봐야 '충주산림조합'·'서울보증보험주식회사'가 자연인 오탐에서 살아남는다.
    """
    w = _strip_josa(word)
    return any(w.endswith(s) for s in _CORP_SUFFIX)
...
def mask_personal_names(text: str) -> str:
    ...
    def _role_first(m: re.Match) -> str:
        rest = m.group(3) or ""
        # (QA 2026-07-26) 전체 단어가 법인·기관이면 보존 — '한국전력공사'는 앞 4자만 보면
        # 자연인처럼 보이지만 전체 단어의 접미사(공사)가 정체를 말해준다.
        if rest and _corp_word(m.group(2) + rest):
            return m.group(0)
        # (QA 2026-07-26) 뒤에 '(주'가 바로 붙으면 법인 — "근저당권자 지에스칼텍스(주)" 보존.
        if m.string[m.end(3):m.end(3) + 2] == "(주":
            return m.group(0)
        ...
        if tail:
            def _item(x: re.Match) -> str:
                irest = x.group(3) or ""
                if irest and _corp_word(x.group(2) + irest):
                    return x.group(0)
                if not _looks_like_name(x.group(2)):
                    return x.group(0)
                _, j = _split_name_josa(x.group(2))
                return f"{x.group(1)}[성명]{j}{irest}"
            tail = _PII_NAME_LIST_RE.sub(_item, tail)
        return f"{m.group(1)} [성명]{josa}{rest}{tail}"
