...
### [done] T7. UI 메시지·포지셔닝 전환 ✅ 사이클#7 평가자 PASS (NEEDS_WORK 1회 → 재작업 → PASS)
- 왜: "예상 차익 2.3억"(단정) → "보수 가격 기준 차익 1.4억 · 실거래 6건 기준"(신뢰 고지).
- 완료 정의:
  - [x] digest TOP + 목록 히어로 스포트라이트에서 '위험' 제외(테이블에는 경고뱃지와 잔존 — 배제 아님)
  - [x] 목록(히어로·셀·헤더·푸터)/상세(히어로·푸터)/비교/digest markdown 전부 profit_low 중심 "보수 기준" 전환, query.py 정렬·min_profit도 보수 기준
  - [x] "실거래 N건 기준"·scope 근거 병기(상세 히어로는 '같은 단지/같은 평형'까지)
  - [x] "권리 확인 완료 전까지 최종 판단 금지" 고지 — 상세 페이지 + digest 푸터
  - [x] README "차익이 확실한"·"사라고 콕 집어주는 엔진" 제거 → "초보자를 위한 아파트 경매 1차 필터", 타이틀·백링크 6곳 네이밍 정리
  - [x] pytest 335 passed(+히어로 회귀가드 2) + ruff 클린 + evidence/t7_ui_copy.txt 21/21 OK
