...
# 단계 → 로그에 찍히는 표식. 리스트 스윕은 전용 표식이 없어 '시작했고 [2/5] 전'으로 판정한다.
# (2026-07-31) 네이버가 맨 앞 → [4/5]로 내려가며 4단계 → 5단계가 됐다(느린 네이버가 시간
# 예산을 다 먹어 경매 크롤이 굶던 사고. refresh-daily.ps1 [1/5] 앞 주석 참조).
