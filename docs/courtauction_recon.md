...
### 라이브 검증 상태 (미완 — ⚠️ 소프트차단으로 보류)
- `csNo="2022타경3289"+auctnInfOriginDvsCd="2"` 단독 POST → **200이지만 `{ipcheck:false}` 빈 응답**.
- **가설 테스트(2026-07-22 11:xx)**: "상세조회 선행 → 현황조사서" 순서를 client 안전경로로 3건 시도.
  그러나 **상세조회(case_detail) 자체가 3건 모두 `dma_result 없음`으로 실패** — 같은 세션 초반엔
  동일 호출이 정상 반환했으므로 코드회귀 아니라 **당일 누적요청(XML 정찰 15+콜)으로 WAF 소프트차단**.
  회로차단기가 위장차단 감지하고 정상 중단. → 밴 경계에 닿아 **courtauction 호출 전면 중단**.
- **다음 세션 재시도 규칙**: ① 최소 수 시간(가능하면 하루) 경과 후 ② **권리 크롤과 동시 금지**
  (IP 공유로 요청 합산) ③ 크롤 완전 휴지기에 **단 1건**으로 "상세1콜→curst1콜" 순서 검증.
  성공하면 그때 통합. (참고: `selectPopUpGdsLstDts.on`은 물건 목록내역이며 임대차 정보 없음 —
  XML summary 라벨 '부동산 임대차 정보'는 오표기)
